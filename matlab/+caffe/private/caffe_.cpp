...
static vector<shared_ptr<P2PSync<float> > > syncSolvers_;
...
static int handle_to_ID(const mxArray* mx_handle) {
	mxArray* mx_ID = mxGetField(mx_handle, 0, "ID");
	mxArray* mx_init_key = mxGetField(mx_handle, 0, "init_key");
	double t = mxGetScalar(mx_ID);
	mxCHECK(t >= 0, "Net ID should be nonnegative integer.");
	mxCHECK(mxGetScalar(mx_init_key) == init_key,
		"Could not convert handle to pointer due to invalid init_key. "
		"The object might have been cleared.");
	return t;
...
static void select_solver(MEX_ARGS){
	mxCHECK(nrhs == 1 && mxIsStruct(prhs[ 0 ]),
		"Usage: caffe_('select_solver', hSolver)");
...
static mxArray* create_handle_vec(int ptr_num) {
  const int handle_field_num = 3;
  const char* handle_fields[handle_field_num] = { "ptr", "ID", "init_key" };
...
static void setup_handle(const T* ptr, int index, mxArray* mx_handle_vec) {
  ...
  *reinterpret_cast<uint64_t*>(mxGetData(mx_ptr)) =
  ...
  mxSetField(mx_handle_vec, index, "ID", mxCreateDoubleScalar(now_solver_));
}
...
static void get_solver(MEX_ARGS) {
  ...
  // Get solver id
  int temp_id = 0;
  while ( syncSolvers_.size() > temp_id && syncSolvers_[ temp_id ] )
  {
	  temp_id++;
  }
  now_solver_ = temp_id;
  ...
  if ( iDeviceNum <= 0 )
	  mexErrMsgTxt("caffe::Do not find CUDA devices.");
  
  vector<int> gpu_t;
  if ( nrhs == 2 )
  {
	  const double* const data_ptr = reinterpret_cast<const double* const>( mxGetPr(prhs[ 1 ]) );
	  const int ele_num = mxGetNumberOfElements(prhs[ 1 ]);
	  for ( int i = 0; i < ele_num; ++i )
	  {
		  int device_id = int(data_ptr[ i ]);
		  if ( device_id >= iDeviceNum )
			  mexErrMsgTxt("caffe::Device_id should in [0, gpuDeviceCount-1]");
		  gpu_t.push_back(device_id);
	  }
  }
  else{
	  gpu_t.push_back(0);
  ...
  if ( syncSolvers_.size() <= temp_id )
	  gpu_groups_.push_back(gpu_t);
  else
	  gpu_groups_[ now_solver_ ] = gpu_t; 
  Caffe::SetDevice(gpu_groups_[ now_solver_ ][ 0 ]);
  Caffe::set_mode(Caffe::GPU);
  Caffe::set_solver_count(gpu_t.size()); 
  // Creat solver
  ...
  // Creat P2PSync
  boost::shared_ptr<caffe::P2PSync<float>> sync_t(new caffe::P2PSync<float>(solver, nullptr, solver->param()));
  if ( syncSolvers_.size() <= temp_id )
  {
	  syncSolvers_.push_back(sync_t);
  }
  else{
	  syncSolvers_[ now_solver_ ] = sync_t;
  }
  syncSolvers_[ now_solver_ ]->pre_run(gpu_groups_[ now_solver_ ]);
#pragma omp parallel num_threads(int(gpu_groups_[ now_solver_ ].size())) 
  //for ( int ID = 0; ID < int(gpu_groups_[ now_solver_ ].size()); ID++ )
  {
	  int ID = omp_get_thread_num();
	  Caffe::set_mode(Caffe::GPU);
	  Caffe::set_solver_count(gpu_groups_[ now_solver_ ].size());
	  if ( ID == 0 )
		  gpu_groups_[ now_solver_ ][ ID ] = syncSolvers_[ now_solver_ ]->solver()->param().device_id();
	  else
		  gpu_groups_[ now_solver_ ][ ID ] = syncSolvers_[ now_solver_ ]->workers()[ ID ]->solver()->param().device_id();
  ...
  plhs[ 0 ] = ptr_to_handle<P2PSync<float> >(sync_t.get());
  mxFree(solver_file);
}
...
static void solver_get_attr(MEX_ARGS) {
  ...
  mxArray* top;
  now_solver_ = handle_to_ID(prhs[ 0 ]);
  top = mxCreateCellMatrix(int(gpu_groups_[now_solver_].size()), 1);
  ...
  mxSetField(mx_solver_attr, 0, "hNet_net",
	  ptr_to_handle<Net<float> >(p2psync->solver()->net().get()));
  mxSetField(mx_solver_attr, 0, "hNet_test_nets",
	  ptr_vec_to_handle_vec<Net<float> >(p2psync->solver()->test_nets()));
  mxSetCell(top, 0, mx_solver_attr);
  for ( int i = 1; i < gpu_groups_[ now_solver_ ].size(); i++ )
  {
	  mxArray* mx_solver_attr = mxCreateStructMatrix(1, 1, solver_attr_num,
		  solver_attrs);
	  mxSetField(mx_solver_attr, 0, "hNet_net",
		  ptr_to_handle<Net<float> >(p2psync->workers()[i]->solver()->net().get()));
	  mxSetField(mx_solver_attr, 0, "hNet_test_nets",
		  ptr_vec_to_handle_vec<Net<float> >(p2psync->workers()[ i ]->solver()->test_nets()));
	  mxSetCell(top, i, mx_solver_attr);
  }
  plhs[ 0 ] = top;
}
...
static void solver_get_iter(MEX_ARGS) {
  mxCHECK(nrhs == 1 && mxIsStruct(prhs[0]),
      "Usage: caffe_('solver_get_iter', hSolver)");
  P2PSync<float>* p2psync = handle_to_ptr<P2PSync<float> >(prhs[ 0 ]);
  plhs[ 0 ] = mxCreateDoubleScalar(p2psync->solver()->iter());
}
...
static void solver_restore(MEX_ARGS) {
  ...
  p2psync->solver()->Restore(snapshot_file);
  mxFree(snapshot_file);
}
...
static void solver_solve(MEX_ARGS) {
  ...
  P2PSync<float>* p2psync = handle_to_ptr<P2PSync<float> >(prhs[ 0 ]);
  now_solver_ = handle_to_ID(prhs[ 0 ]);
  p2psync->Run(gpu_groups_[ now_solver_ ]);
...
static void solver_step(MEX_ARGS) {
  mxCHECK(nrhs == 2 && mxIsStruct(prhs[0]) && mxIsDouble(prhs[1]),
      "Usage: caffe_('solver_step', hSolver, iters)");
  P2PSync<float>* p2psync = handle_to_ptr<P2PSync<float> >(prhs[ 0 ]);
  Caffe::set_solver_count(gpu_groups_[ handle_to_ID(prhs[ 0 ]) ].size());
  mxCHECK(p2psync, "Solver not init.");
  now_solver_ = handle_to_ID(prhs[ 0 ]);
  do_set_phase(TRAIN);
...
#pragma omp parallel num_threads(int(gpu_groups_[now_solver_].size())) 
	{
		...
		for ( int i_t = 0; i_t < input_blobs.size(); i_t++ ){
			...
			if ( mxGetNumberOfElements(elem) != input_blobs[ i_t ]->count() ){
				char t[ 1000 ];
				sprintf(t, "caffe_ : solver_reshape_input(net):input size should be equal to blob size clust %d: %d vs %d\n" , ID, mxGetNumberOfElements(elem) , input_blobs[ i_t ]->count() );
				mexErrMsgTxt(t);
			}
			double* shape_mem_mtr = mxGetPr(elem);
			const int num_axes = mxGetNumberOfElements(elem);
...
static void set_device(MEX_ARGS) {
  mxERROR("Please instead this interface with \"get_solver\".");
